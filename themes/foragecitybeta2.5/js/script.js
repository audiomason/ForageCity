			...
			success: function(data, status, xhr){
				...
				// PROFILE FUNCTIONS
					...
					// DISPLAY EDIT FORM
						...
						en = u.find("email_notifications").text();
		...
		sw.delegate("input[type=submit]", clte, function(e, undefined){
			...
			if(claimTapLock(this) && !tm){
				...
				var t = jQuery(this), form = t.parents("form").first(), contentDiv = jQuery("div#content"), errMsg = "",
					bn, bp, cq, gn, gt, prob, gu, guo, guov, inst, instv, desc, descv, q, qv, l, lv, la, lav, lo, lov, em, gi, gq, en, xtraNames, xtraValues;
				if(t.is("div.find-box div.box-info input")){
				   ...
				   get_wp_data('GET', data);
					...
					en = form.find("input[name=email_notifications]:checked").val();
					if(en != "yes") en = "no";
					data = {
			...
			myScroll = new iScroll('scroll-wrapper', {
				...
				onBeforeScrollStart: function (e) {
					if(iOS){
						...
						if (navigator.userAgent.match(/OS [1-4]_\d like Mac OS X/i)){
							e.preventDefault();
						}else{
